...
FinalAction.prototype.getResource = function(nextStep, callback) {
  ...
  this.walker.process(nextStep, function(err, step) {
    ...
    if (!self.walker.checkHttpStatus(step, callback)) return;
    var doc = self.walker.parse(step, callback);
    if (!doc) return;
...
FinalAction.prototype.walkAndExecute =
  ...
  walkState.callback = function(err, step) {
    ...
    if (util.isArray(walkState.requestOptions)) {
      options = walkState.requestOptions[step.index] || {};
    }
    log.debug('executing final request with step: ' + JSON.stringify(step));
    log.debug('options: ' + JSON.stringify(options));
    self.executeRequest(step.uri, request, options, method, walkState.body,
      callback);
  };
