...
Builder.prototype.newRequest = function() {
  ...
  clonedRequestBuilder.convertResponseToObjectFlag =
    this.convertsResponseToObject();
 ...
 * The default false, which means the full response object is handed to the
 * callback.
 *
 ...
 *
 * Response body conversion is currently only supported for JSON media types.
 *
 ...
 * undefined), automatic conversion is switched off.
 */
 ...
 * JavaScript before passing the result back to the callback.
 */
...
function createInitialTraversalState(self, body) {
  ...
  var traversalState = {
    ...
    continuation: null,
    convertResponseToObjectFlag: self.convertsResponseToObject(),
    links: self.links,
...
function initFromTraversalState(self, t) {
  ...
  self.convertResponseToObjectFlag = t.convertResponseToObjectFlag;
  self.links = [];
