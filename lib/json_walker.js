...
/*
 * Fetches the document from the startUri and then, starting with the first
 * element from the links array:
 ...
 *    given templateParams, otherwise it is interpreted as an URI as is.
 * 4) Passes the resulting URI to this.get to acquire the next document.
 * 5) Goes back to step 1) with the next element from the links array, if any.
 *
 * When the link array has been consumed completely by the above procedure),
 ...
 *      called on RequestBuilder, this will be a get/post/put/... request),
 *    - lastUri: the last URI that JsonWalker has accessed,
 *    - lastResponse: the HTTP response from accessing lastUri
 *
 * Parameters:
 * - startUri is the first URI to get (usually the root URI of the API)
 ...
 * - callback will be called if this method is done.
 */
