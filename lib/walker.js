  ...
  function executeNextStep() {
    if (walkState.nextStep.index < self.links.length &&
      ...
      self.process(walkState.nextStep, function(err, lastStep) {
        ...
        if (!self.checkHttpStatus(walkState.lastStep, callback)) return;

        // parse JSON from last response
        var doc = self.parse(walkState.lastStep, callback);
        if (!doc) return;
        ...
        walkState.nextStep = self.findNextStep(doc, link, callback);
        if (!walkState.nextStep) return;
        ...
        if (walkState.nextStep.uri) {
          ...
          walkState.nextStep.uri = self.resolveUriTemplate(walkState.nextStep,
