...
RequestBuilder.prototype.post = function(body, callback) {
  this.putPost(body, request.post, callback)
}

RequestBuilder.prototype.put = function(body, callback) {
  this.putPost(body, request.put, callback)
}

RequestBuilder.prototype.putPost = function(body, method, callback) {
  var self = this
  var walker = createWalker(this.mediaType)
  if (!walker) {
    return mediaTypeError(callback, this.mediaType)
...
RequestBuilder.prototype.executeRequest = function(uri, method, body,
    callback) {
  log.debug('request to ' + uri + ' with ' + body)
  method(uri, { body: body }, function(err, response) {
    log.debug('request to ' + uri + ' with ' + body + ' succeeded')
