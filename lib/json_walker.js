    ...
    if (index < links.length) {
      ...
      self.get(nextUri, function(err, response) {
        ...
        log.debug('request to ' + lastUri + ' finished (' +
            response.statusCode + ')')
        if (err) { return callback(err, null, lastUri, lastResponse) }
        //log.debug('extracting ' + JSON.stringify(lastResponse))
        self.extract(lastResponse, lastUri, function(err, doc) {
          log.debug('extracted ' + JSON.stringify(doc))
          if (err) { return callback(err, null, lastUri, lastResponse) }

          var link = links[index++]
          nextUri = extractLink(doc, link)
          if (util.isError(nextUri)) {
            return callback(nextUri, null, lastUri, lastResponse)
          } else if (!nextUri) {
            return callback(new Error('Could not find property ' + link +
                  ' in document:\n' + doc), null, lastUri, lastResponse)
          } else {
      ...
      // TODO
      // Pass last accessed URI back as well as
      // next-to-access URI
      // also last response
      // but do not execute last request, RequestBuilder will have to do that
      // Wrap all information up in one object
      // {
      //   lastUri: ...,
      //   nextUri: ...,
      //   lastResponse: ,,,
      // }
      //
      // This needs to be changed in quite a few places
      callback(null, nextUri, lastUri, lastResponse)
    }
  })()
}
...
JsonWalker.prototype.resolveJSONPath = function(link, doc) {
  ...
  if (matches.length === 1) {
    var uri = matches[0]
    if (!uri) {
      return new Error('JSONPath expression ' + link +
