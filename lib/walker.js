...
Walker.prototype.walk = function(callback) {
  ...
  walkState.nextStep.uri = this.resolveUriTemplate(walkState.nextStep,
...
Walker.prototype._executeNextStep = function(ws) {
  var self = this;
  if (ws.nextStep.index < self.links.length &&
      !self.aborted) {

    // Trigger execution of next step. In most cases that is an HTTP get to
    // the next URI.
    self.process(ws.nextStep, function(err, lastStep) {
      ws.finalStep = ws.lastStep = lastStep;
      if (err) {
        log.debug('error while processing step ' +
          JSON.stringify(ws.lastStep));
        log.error(err);
        return ws.callback(err, ws.nextStep, ws.lastStep);
      }
      log.debug('successfully processed step');

      // check HTTP status code
      if (!self.checkHttpStatus(ws.lastStep, ws.callback)) return;

      // parse JSON from last response
      var doc = self.parse(ws.lastStep, ws.callback);
      if (!doc) return;

      // extract next link to follow from last response
      var link = self.links[ws.lastStep.index];
      log.debug('next link: ' + link);

      ws.nextStep = self.findNextStep(doc, link, ws.callback);
      if (!ws.nextStep) return;
      ws.nextStep.index = lastStep.index + 1;
      ...
      if (ws.nextStep.uri) {
        ...
        ws.nextStep.uri = self.resolveUriTemplate(ws.nextStep,
            self.templateParameters);
      }

      // turn relative URI into absolute URI or whatever else is required
      self.postProcessStep(ws.nextStep, ws.lastStep);
      log.debug('next step: ' + JSON.stringify(ws.nextStep, null, 2));
    ...
    return ws.callback(null, ws.nextStep, ws.finalStep);
  }
};
...
Walker.prototype.get = function(step, callback) {
  ...
  self.currentRequest =
    ...
    self.detectContentType(response, step, callback);
...
Walker.prototype.detectContentType = function(response, step, callback) {
