...
Walker.prototype.walk = function(walkState, callback) {
  walkState.step.uri = resolveUriTemplate(walkState);
...
Walker.prototype._executeNextStep = function(ws) {
  ...
  if (ws.step.index < ws.links.length && !self.aborted) {
    ...
    // the next URI.
    // TODO Extract callback into non-anonymous function
    self.process(ws, function(err, newWs) {
      ...
      ws.lastStep = ws.step;
      ws.step = self.findNextStep(ws, doc, link);
      ...
      if (ws.step.uri) {
        ...
        ws.step.uri = resolveUriTemplate(ws);
      }
...
Walker.prototype.findNextStep = function(ws, doc, link) {
  try {
