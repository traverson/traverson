...
var abortTraversal = require('./abort_traversal').abortTraversal
  , FinalAction = require('./final_action')
...
Builder.prototype.get = function(callback) {
  ...
  walkState.callback = function(err) {
    log.debug('walker.walk returned');
    if (err) {
      return callback(err);
    }
    self.finalAction.get(walkState, callback);
  };
...
Builder.prototype.getResource = function(callback) {
  ...
  walkState.callback = function(err) {
    ...
    self.finalAction.getResource(walkState, callback);
  };
  this.walker.walk(walkState);
  return createTraversalHandle(walkState);
};
...
Builder.prototype.getUri = function(callback) {
  ...
  walkState.callback = function(err) {
    ...
    self.finalAction.getUri(walkState, callback);
  };
  this.walker.walk(walkState);
  return createTraversalHandle(walkState);
};
...
Builder.prototype.post = function(body, callback) {
  ...
  this.finalAction.walkAndExecute(walkState,
