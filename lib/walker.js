...
Walker.prototype._executeNextStep = function(ws) {
  ...
  if (ws.step.index < ws.links.length && !self.aborted) {
    ...
    self.process(ws, function(err, newWs) {
      ...
      if (!self.checkHttpStatus(ws)) return;
...
Walker.prototype.checkHttpStatus = function(ws) {
...
Walker.prototype.postProcessStep = function(ws) {
  if (ws.step.uri) {
    if (ws.step.uri.search(protocolRegEx) !== 0) {
      log.debug('found non full qualified URL');
      if (this.resolveRelative && ws.lastStep && ws.lastStep.uri) {
