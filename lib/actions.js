...
function afterGet(t, callback) {
  ...
  httpRequests.fetchResource(t, function(err, t) {
    ...
    if (err) {
      return callback(err, responseFromState(t), uriFromState(t));
    }
 ...
 */
exports.getUri = function(t, callback) {
  t.callback =
    createWalkStateCallback(t, afterGetUri, callback);
  walker.walk(t);
  return createTraversalHandle(t);
};

function afterGetUri(t, callback) {
  log.debug('returning uri');
  if (t.step.uri) {
    return callback(null, t.step.uri);
  } else if (t.step.doc &&
    // TODO actually this is very HAL specific :-/
    t.step.doc._links &&
    t.step.doc._links.self &&
    t.step.doc._links.self.href) {
    return callback(null, t.startUri + t.step.doc._links.self.href);
  } else {
...
function uriFromState(t) {
  return t ? (t.step ? t.step.uri : null) : null;
}
