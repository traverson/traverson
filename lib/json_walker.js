  ...
  (function executeNextRequest() {
    if (index < links.length) {
      ...
      self.get(nextUri, function(err, response) {
        ...
        //log.debug('parseing ' + JSON.stringify(lastResponse))
        self.parse(lastResponse, lastUri, function(err, doc) {
          log.debug('parsed ' + JSON.stringify(doc))
          if (err) { return callback(err, result(null, lastUri, lastResponse)) }
...
JsonWalker.prototype.parse = function(response, uri, callback) {
